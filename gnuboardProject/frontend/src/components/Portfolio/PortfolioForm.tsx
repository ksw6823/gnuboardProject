...
// --- PortfolioCreate.tsx 코드 전체 복사 시작 ---
// (아래는 PortfolioCreate.tsx의 전체 코드이며, 수정모드용으로 일부 변경됨)

// ... (PortfolioCreate.tsx의 스타일 컴포넌트 정의 부분은 동일하게 복사) ...
// (생략)
...
const PortfolioForm: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchPortfolio = async () => {
      try {
        const { data } = await axios.get(`${process.env.REACT_APP_API_URL}/portfolios/${id}`, {
          headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` },
        });
        setTitle(data.title || '');
        setIntro(data.intro || '');
        // sections → 각 state로 분기 및 태그 세팅
        if (data.sections && data.sections.length > 0) {
          // skills
          const skillIds = data.sections.flatMap(
            (s: any) => (s.portfolioSkills || []).map((ps: any) => ps.skillId)
          );
          setSelectedSkillIds(skillIds);

          // keywords
          const keywordIds = data.sections.flatMap(
            (s: any) => (s.portfolioKeywords || []).map((pk: any) => pk.keywordId)
          );
          setSelectedKeywordIds(keywordIds);

          // job (여러 개일 수 있지만, 첫 번째만 사용)
          const jobIds = data.sections.flatMap(
            (s: any) => (s.portfolioJob || []).map((pj: any) => pj.jobId)
          );
          setSelectedJobId(jobIds.length > 0 ? jobIds[0] : null);

          setEducations(
            data.sections.filter((s: SectionDto) => s.type === 'education').map((s: SectionDto) => {
          ...
          setExperiences(
            data.sections.filter((s: SectionDto) => s.type === 'experience').map((s: SectionDto) => {
              const content = JSON.parse(s.content!);
              return {
                ...content,
                period: {
                  startDate: content.period?.startDate ? new Date(content.period.startDate) : null,
                  endDate: content.period?.endDate ? new Date(content.period.endDate) : null,
                },
                isConfirmed: true
              };
            })
          ...
          setProjects(
            data.sections.filter((s: SectionDto) => s.type === 'project').map((s: SectionDto) => {
              const content = JSON.parse(s.content!);
              return {
                ...content,
                period: {
          ...
          setCertificates(
            data.sections.filter((s: SectionDto) => s.type === 'certificate').map((s: SectionDto) => ({ ...JSON.parse(s.content!), isConfirmed: true }))
          );
          setLanguages(
            data.sections.filter((s: SectionDto) => s.type === 'language').map((s: SectionDto) => ({ ...JSON.parse(s.content!), isConfirmed: true }))
          );
          setActivities(
            data.sections.filter((s: SectionDto) => s.type === 'activity').map((s: SectionDto) => {
              const content = JSON.parse(s.content!);
              return {
                ...content,
                period: {
        ...
        setErrorMsg('포트폴리오 데이터를 불러오지 못했습니다.');
      }
    };
    fetchPortfolio();
  }, [id]);
  ...
  const handleSave = async () => {
    ...
    // Section 데이터 준비 (PortfolioCreate와 동일)
    const sections: any[] = [];
    educations.filter(e => e.isConfirmed && e.school).forEach(e => {
      const { isConfirmed, ...rest } = e;
      sections.push({
        type: 'education',
        content: JSON.stringify({
          ...rest,
          startDate: rest.startDate instanceof Date && !isNaN(Number(rest.startDate)) ? rest.startDate.toISOString().slice(0, 10) : null,
          endDate: rest.endDate instanceof Date && !isNaN(Number(rest.endDate)) ? rest.endDate.toISOString().slice(0, 10) : null,
        })
      });
    });
    experiences.filter(e => e.isConfirmed && e.company && e.position && e.period?.startDate).forEach(e => {
      ...
      sections.push({
        ...
        content: JSON.stringify({
          ...rest,
          period: {
            startDate: rest.period && rest.period.startDate instanceof Date && !isNaN(Number(rest.period.startDate)) ? rest.period.startDate.toISOString().slice(0, 10) : null,
            endDate: rest.period && rest.period.endDate instanceof Date && !isNaN(Number(rest.period.endDate)) ? rest.period.endDate.toISOString().slice(0, 10) : null
          }
        })
      });
    });
    projects.filter(p => p.isConfirmed && p.name && p.period?.startDate).forEach(p => {
      const { isConfirmed, ...rest } = p;
      sections.push({
        type: 'project',
        content: JSON.stringify({
          ...rest,
    ...
    certificates.filter(c => c.isConfirmed && c.name).forEach(c => {
      const { isConfirmed, ...rest } = c;
      sections.push({ type: 'certificate', content: JSON.stringify(rest) });
    });
    languages.filter(l => l.isConfirmed && l.name).forEach(l => {
      const { isConfirmed, ...rest } = l;
      sections.push({ type: 'language', content: JSON.stringify(rest) });
    });
    activities.filter(a => a.isConfirmed && a.name && a.org && a.period?.startDate).forEach(a => {
      const { isConfirmed, ...rest } = a;
      sections.push({
        type: 'activity',
        content: JSON.stringify({
          ...rest,
  ...
  return (
    <>
      ...
      <FixedSaveButton onClick={handleSave}>수정 완료</FixedSaveButton>
...
// --- PortfolioCreate.tsx 코드 전체 복사 끝 --- 